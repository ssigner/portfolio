...
const caseStudies = [
  {
    title: "초기 접근 방식 전환: Segment Anything → YOLOv8",
    problem:
      "처음에는 Segment Anything을 활용해 박스 영역을 추출하는 방식을 검토했지만, 모델 크기가 크고 추론 시간이 길어 공모전 요구사항과 서버 환경에 맞지 않았습니다. 또한 주최 측 요구사항상 사용자가 터치 포인트를 지정하는 방식이 아니라, 사진 촬영만으로 분석이 이루어져야 했습니다.",
    solution:
      "사용자 상호작용이 필요한 세그멘테이션 방식 대신 YOLOv8 기반 박스 검출 방식으로 방향을 전환했습니다. 이후 박스 crop, 배경 제거, Canny Edge Detection, 꼭지점 좌표 추출, 카메라 기하 계산으로 이어지는 파이프라인을 구성했습니다.",
    result:
      "기술 선택을 요구사항에 맞게 조정하면서, 모델 검출 결과를 실제 박스 치수 계산으로 연결할 수 있는 구조를 만들었습니다.",
  },
  {
      ...
      <section className="mx-auto max-w-6xl px-6 py-20">
        ...
        <section className="mt-16 rounded-2xl border border-emerald-300/40 bg-zinc-900/60 p-8 shadow-[0_0_40px_rgba(16,185,129,0.08)]">
          ...
          <p className="mt-6 max-w-4xl leading-8 text-zinc-300">
            ...
            실제 크기를 추정하는 것을 목표로 했습니다. 초기에는 Segment Anything을
            활용한 박스 영역 추출을 검토했지만, 모델 크기와 추론 시간, 서버 환경,
            사용자 상호작용 제한 문제로 인해 YOLOv8 기반 박스 검출 방식으로 전환했습니다.
            이후 박스 영역 검출, 배경 제거, 외곽선 추출, 꼭지점 좌표 계산, 카메라
            기하 기반 치수 계산까지 이어지는 파이프라인을 구성했습니다.
          </p>
        </section>
...
Width / Depth / Height Estimation`}
          ...
          <p className="mt-6 max-w-4xl leading-8 text-zinc-300">
            ...
            박스의 형태를 좌표 데이터로 변환합니다. OpenCV의 GrabCut과 Canny Edge
            Detection, morphology 처리를 통해 박스 윤곽선을 추출하고, contour 기반으로
            꼭지점 좌표를 계산했습니다. 마지막 단계에서는 solvePnP로 카메라 외부
            파라미터를 추정하고, 이미지 상의 2D 좌표와 카메라-박스 간 거리 관계를
            활용해 실제 월드 좌표계의 박스 크기를 추정했습니다.
          </p>
        </section>
